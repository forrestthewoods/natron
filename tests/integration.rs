...
/// A/B regression: confirm `extract_msi_pure` produces a byte-identical
/// `Windows Kits/10/` subtree to the legacy `extract_msi` (msiexec /a)
/// for the 7 default essential MSIs of the newest SDK on the live mirror.
///
/// `#[cfg(windows)]` because the comparison baseline (`extract_msi`)
/// only exists on Windows. This test is scaffolding for the
/// cross-platform MSI migration and is intended to be **deleted** once
/// the new extractor has shipped through a release cycle as the
/// production path — see the plan + the migration commit messages.
#[test]
#[cfg(windows)]
#[ignore = "network+windows: A/B-validates pure-Rust MSI extractor vs msiexec"]
fn test_msi_ab_extract_matches_msiexec() {
    use natron::extract;
    use natron::providers::InstallCtx;
    use natron::providers::vs_manifest::MirrorUrls;
    use natron::providers::windows_sdk;
    ...
    let versions = windows_sdk::discover_sdk_versions(&urls, &ctx).expect("discover");
    let newest = versions.first().expect("at least one SDK").clone();
    eprintln!("A/B test target: SDK {newest}");
    let resolved =
        windows_sdk::resolve_sdk_version(&urls, &newest, &ctx).expect("resolve");

    // Find the SDK component meta-package and walk its deps directly so
    // the test isn't coupled to provider-internal helpers.
    let component = resolved
        .manifest
        .packages
        .iter()
        .find(|p| p.id == resolved.sdk_pkg_id)
        .expect("SDK component package present in manifest");
    let dep_ids: Vec<String> = component.dependencies.keys().cloned().collect();

    // Stage every dep's payloads (MSIs + CABs) flat into one dir. This
    // mirrors what windows_sdk::install does so msiexec can resolve CAB
    // siblings by basename, and our pure extractor sees the same layout.
    let payloads_dir = env.temp.path().join("payloads");
    std::fs::create_dir_all(&payloads_dir).unwrap();
    let mut msis_to_extract: Vec<PathBuf> = Vec::new();
    for dep_id in &dep_ids {
        let Some(pkg) = resolved
            .manifest
            .packages
            .iter()
            .find(|p| &p.id == dep_id)
        ...
        for p in &pkg.payloads {
            let raw_filename = p
                ...
                .clone()
                .or_else(|| {
                    url::Url::parse(&p.url).ok().and_then(|u| {
                        u.path_segments()
            ...
            let basename = windows_sdk::strip_installer_prefix(&raw_filename);
            let downloaded = ctx
            ...
            if !dest.exists() {
                ...
                r.expect("stage payload");
            }
            if raw_filename.to_lowercase().ends_with(".msi")
                && windows_sdk::DEFAULT_ESSENTIAL_MSIS
                    .iter()
    ...
    assert!(
        !msis_to_extract.is_empty(),
        "expected at least one default-group MSI for SDK {newest}"
    );
    msis_to_extract.sort();
    msis_to_extract.dedup();
    eprintln!("extracting {} MSIs both ways", msis_to_extract.len());

    let extract_old = env.temp.path().join("extract_old");
    let extract_new = env.temp.path().join("extract_new");
    std::fs::create_dir_all(&extract_old).unwrap();
    std::fs::create_dir_all(&extract_new).unwrap();
    for msi in &msis_to_extract {
        extract::extract_msi(msi, &extract_old)
            .unwrap_or_else(|e| panic!("msiexec extract {}: {e:#}", msi.display()));
        extract::extract_msi_pure(msi, &extract_new)
            .unwrap_or_else(|e| panic!("pure extract {}: {e:#}", msi.display()));
    }

    if let Err(diff) = common::diff_trees(&extract_old, &extract_new) {
        panic!(
            "extract_msi_pure output diverges from msiexec for SDK {newest}:\n{diff}"
        );
    }
    eprintln!("A/B match confirmed for SDK {newest}");
}
